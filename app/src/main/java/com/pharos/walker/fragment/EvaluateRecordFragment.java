        ...
        evaluateRecordAdapter.setOnItemClickListener((view, position, viewType, lastSelected) -> {
            if (viewType == catRecord) {
                ...
                if (lastSelected != -1 && lastSelected != position){
                    LinearLayout linearLayout = (LinearLayout) Objects.requireNonNull(rvSheet.getLayoutManager()).findViewByPosition(lastSelected);
                    TextView lastTextView = null;
                    if (linearLayout != null) {
                        lastTextView = (TextView) linearLayout.findViewById(R.id.tv_sheetRow4);
                    }
...
//        data.setValueTypeface(Typeface.DEFAULT_BOLD);
        ...
        yRightAxis.setAxisMaximum(data.getYMax() + 5);
