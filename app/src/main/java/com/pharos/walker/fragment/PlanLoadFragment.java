...
public class PlanLoadFragment extends Fragment {
    ...
    private List<Integer> initLoadList;
    private int axisFontSize = 22;
    private int lastFinishPosition = 0;
//    public PlanLoadFragment(List<SubPlanEntity> subPlanEntityList, List<OriginalSubPlanEntity> originalSubPlanEntityList) {
//        this.subPlanEntityList = subPlanEntityList;
//        this.originalSubPlanEntityList = originalSubPlanEntityList;
//    }
    public PlanLoadFragment(List<OriginalSubPlanEntity> originalSubPlanEntityList) {
        this.originalSubPlanEntityList = originalSubPlanEntityList;
    }
...
//        this.subPlanEntityList = subPlanEntityList;
    ...
    private void copyLoad(List<SubPlanEntity> subPlanEntityList){
        if (initLoadList == null){
            initLoadList = new ArrayList<>();
        }else {
            initLoadList.clear();
        }
        for (SubPlanEntity subPlanEntity:subPlanEntityList){
            initLoadList.add(subPlanEntity.getLoad());
        }
    }
...
//        chart.setBorderWidth(1f);
                ...
                public boolean onTouch(View v, MotionEvent event) {
                    switch (event.getAction()) {
                        ...
                        case MotionEvent.ACTION_MOVE:
                            if (isMoveChange && lastFinishPosition <= pos) {
                                if (pos >= dataSize - 1) {
                                    pos = dataSize - 1;
                                } else {
        ...
        chart.getViewTreeObserver().addOnGlobalLayoutListener(() -> {
            ...
            android.util.Log.e("train plan", "initChart: 图表高度" + chart.getHeight());
            android.util.Log.e("train plan", "initChart: 图表宽度" + chart.getWidth());
        });
    }
    ...
    private void update(int pos, float f, int isAdd) {
        ...
        if (isAdd == 2) {
            ...
            lastModifyValue = initLoadList.get(pos);
            ...
            subPlanEntity.setLoad((int) MaxWeight);
            subPlanEntity.setTrainStep(Global.MaxTrainStep);
            TrainPlanFragment.subPlanEntityList.remove(TrainPlanFragment.subPlanEntityList.size() - 1);
            TrainPlanFragment.subPlanEntityList.remove(TrainPlanFragment.subPlanEntityList.size() - 1);
            TrainPlanFragment.subPlanEntityList.add(subPlanEntity);
            setChart();
    ...
    private void modifyData(int modifyPosition,int modifyValue){
        int frontIndex = 0;
        int behindIndex = 0;
        int startLoad = TrainPlanFragment.subPlanEntityList.get(lastFinishPosition).getLoad();
        ...
        behindMaxIndex = (dataSize-1 - modifyPosition);
         if (modifyPosition > lastFinishPosition){
             frontHalfDiff = (modifyValue - startLoad)*1.0f/frontMaxIndex;
//             frontHalfDiff = modifyValue - lastModifyValue;
         }
//        android.util.Log.e("planLoadFragment", "modifyData diff: " + frontHalfDiff );
//        android.util.Log.e("planLoadFragment", "modifyData: " + modifyValue);
        int finishLoad = TrainPlanFragment.subPlanEntityList.get(dataSize-1).getLoad();
         if (dataSize-1 > modifyPosition){
             behindHalfDiff = (finishLoad - modifyValue)*1.0f/behindMaxIndex;
//             behindHalfDiff = modifyValue - lastModifyValue;
         }
        for (int i = 0; i < dataSize; i++) {
            SubPlanEntity subPlanEntity = TrainPlanFragment.subPlanEntityList.get(i);
//            if ((i == modifyPosition -1) || (i == modifyPosition +1)){
//                int load = initLoadList.get(i);
//                int modifyLoad =  Math.round((load + frontHalfDiff));
//                if (modifyLoad <= 0){
//                    modifyLoad = 0;
//                }
//                if (modifyLoad >= TrainPlanFragment.subPlanEntityList.get(dataSize-1).getLoad()){
//                    subPlanEntity.setLoad(TrainPlanFragment.subPlanEntityList.get(dataSize-1).getLoad());
//                }else {
//                    subPlanEntity.setLoad(modifyLoad);
//                }
//                TrainPlanFragment.subPlanEntityList.set(i,subPlanEntity);
//            }
            if (i >= lastFinishPosition && i < modifyPosition){
//                subPlanEntity.setLoad(modifyLoad);
                subPlanEntity.setLoad(Math.round(startLoad + frontHalfDiff*frontIndex));
//                if (modifyValue > lastModifyValue){
//                    android.util.Log.e("planLoadFragment", "add: " + Math.round((startLoad + frontHalfDiff*frontIndex)));
//                    subPlanEntity.setLoad(Math.round(startLoad + frontHalfDiff*frontIndex));
//                }else if (modifyValue < lastModifyValue){
//                    android.util.Log.e("planLoadFragment", "reduce: " +  Math.round(modifyValue - Math.abs(frontHalfDiff*(frontMaxIndex -frontIndex))));
//                    subPlanEntity.setLoad(Math.round(startLoad + frontHalfDiff*frontIndex));
////                    subPlanEntity.setLoad(Math.round(modifyValue - Math.abs(frontHalfDiff*(frontMaxIndex -frontIndex))));
//                }
            ...
            if (i > modifyPosition && i < dataSize-1){
                behindIndex++;
                subPlanEntity.setLoad(Math.round(modifyValue + behindHalfDiff*behindIndex));
//                if (modifyValue > lastModifyValue){
//                    subPlanEntity.setLoad(Math.round(modifyValue + behindHalfDiff*behindIndex));
//                }else if (modifyValue < lastModifyValue){
//                    subPlanEntity.setLoad(Math.round(modifyValue + behindHalfDiff*behindIndex));
//                }
                if (subPlanEntity.getLoad() >= finishLoad)
                    subPlanEntity.setLoad(finishLoad);
                TrainPlanFragment.subPlanEntityList.set(i,subPlanEntity);
            }
        }
