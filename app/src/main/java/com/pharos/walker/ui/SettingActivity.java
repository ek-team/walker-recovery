...
import com.pharos.walker.beans.ServerEvaluateEntity;
...
import com.pharos.walker.customview.rxdialog.RxDialogSettingPassword;
...
public class SettingActivity extends BaseActivity {
    ...
    private RxClearZeroDialog dialog;
    private ActivationCodeBean localCodeBean;
    private ConcurrentLinkedQueue<UserBean> cacheQueue = null;
    private ConcurrentLinkedQueue<UserBean> evaluateRecordUserQueue = null;
    ...
    protected void initialize(Bundle savedInstanceState) {
        ...
        evaluateRecordUserQueue = new ConcurrentLinkedQueue<>();
    ...
    private void initView() {
        ...
        etDepartment.setText((TextUtils.isEmpty(SPHelper.getSystemSettingDepartment())? "无":SPHelper.getSystemSettingDepartment()));
        etHospital.setText((TextUtils.isEmpty(SPHelper.getSystemSettingHospitalName())? "无":SPHelper.getSystemSettingHospitalName()));
    ...
    private void initManager() {
        ...
        ConnectionInfo mInfo;mInfo = new ConnectionInfo(ip, port);
    ...
    @OnClick({R.id.iv_back, R.id.tv_update, R.id.btn_clear, R.id.layout_music, R.id.tv_version_name,R.id.btn_send, R.id.btn_cloud_send ,
            R.id.iv_edit_name, R.id.tv_save_hospital, R.id.iv_edit_department, R.id.tv_save_department,R.id.btn_select,R.id.btn_restore_password})
...
//                String desDir = Environment.getExternalStorageDirectory()  + File.separator +"Walker2_V34-20221009_Anhui_debug.apk";
...
//                    updateApk();
            ...
            case R.id.btn_clear:
                ...
                startTask();
                if (dialog == null){
                    dialog = new RxClearZeroDialog(this);
                }
...
//                }
            ...
            case R.id.btn_cloud_send:
                if(NetworkUtils.isConnected() && DateFormatUtil.avoidFastClick(2000)){
                    List<UserBean> list = UserManager.getInstance().loadAllByDate();
                    for (UserBean userBean:list){
                        cacheQueue.offer(userBean);
                        evaluateRecordUserQueue.offer(userBean);
                    }
            ...
            case R.id.iv_edit_name:
                etHospital.setText("");
                etHospital.setFocusableInTouchMode(true);
                etHospital.setFocusable(true);
                etHospital.requestFocus();
                tvSaveHospital.setVisibility(View.VISIBLE);
                break;
            case R.id.tv_save_hospital:
                etHospital.setFocusable(false);
                tvSaveHospital.setVisibility(View.GONE);
                SPHelper.saveSystemSettingHospitalName(etHospital.getText().toString());
                break;
            case R.id.iv_edit_department:
                etDepartment.setText("");
                etDepartment.setFocusableInTouchMode(true);
                etDepartment.setFocusable(true);
                etDepartment.requestFocus();
                tvSaveDepartment.setVisibility(View.VISIBLE);
                break;
            case R.id.tv_save_department:
                etDepartment.setFocusable(false);
                tvSaveDepartment.setVisibility(View.GONE);
                SPHelper.saveSystemSettingDepartment(etDepartment.getText().toString());
                break;
            case R.id.btn_restore_password:
                RxDialogSettingPassword settingPasswordDialog = new RxDialogSettingPassword(this);
                settingPasswordDialog.setSureListener(v -> {
                    if (TextUtils.isEmpty(settingPasswordDialog.getInitPassword())){
                        ToastUtils.showShort("请输入原始密码！");
                        return;
                    }
                    if (!settingPasswordDialog.getInitPassword().equals(Global.InitPassword)){
                        ToastUtils.showShort("原始密码错误！");
                        return;
                    }
                    String newPassword = settingPasswordDialog.getNewPassword();
                    String confirmPassword = settingPasswordDialog.getConfirmPassword();
                    ...
                    SPHelper.saveSettingPassword(confirmPassword);
                    ToastUtils.showShort("密码设置成功");
                    settingPasswordDialog.dismiss();
                });
                settingPasswordDialog.setCanceledOnTouchOutside(false);
...
//                List<PlanEntity> planEntityList = gson.fromJson(serverPlanEntity.getData(),type);
                    ...
                    if (planEntityList.size() == 0 || TrainPlanManager.getInstance().comparePlanUpdateDate(DateFormatUtil.getString2Date(planEntityList.get(0).getUpdateDate()),
                        ...
                        OkHttpUtils.postJsonAsync(Api.uploadPlan, gson.toJson(localPlan), new OkHttpUtils.DataCallBack() {
                            ...
                            planEntityList.get(0).getUserId()) == Global.UploadStatus && !MyUtil.isNoPlanUser()){
...
//                                }
    ...
    private void syncTrainRecord(long userId){
        List<UserTrainRecordEntity> recordEntityList = UserTrainRecordManager.getInstance().loadMasterUpdate(userId);
...
//                    }
                    if (pageNum+1 >= listList.size()){
                        EventBus.getDefault().post(new MessageEvent<>(MessageEvent.ACTION_SYNC_TRAIN_DATA_TO_LOCAL_RESULT,isDuplicate? 0 : code));
                    }else {
...
//        });
    ...
    private void getEvaluateRecord(int pageNum){
        OkHttpUtils.getAsync(Api.getEvaluateRecord +"?userid=" +  SPHelper.getUserId() + "&pageSize=" + pageSize + "&pageNum=" + pageNum, true, new OkHttpUtils.DataCallBack() {
            ...
            @Override
            public void requestSuccess(String result) throws Exception {
                Log.e("Setting Activity", "--->评估记录获取结果" + result);
                JSONObject toJsonObj= new JSONObject(result);
                int code = toJsonObj.getInt("code");
                if (code == 0){
//                    EventBus.getDefault().post(new MessageEvent<>(MessageEvent.ACTION_SYNC_EVALUATE_RECORD_TO_LOCAL_RESULT,code));
    ...
    private void syncEvaluateRecord(long userId){
        List<EvaluateEntity> recordEntityList = EvaluateManager.getInstance().loadNotUploadRecord(userId);
    ...
    private void uploadEvaluateData( int pageNum,List<List<EvaluateEntity>> listList){
        ...
        OkHttpUtils.postJsonAsync(Api.uploadEvaluateRecord, gson.toJson(listList.get(pageNum)), new OkHttpUtils.DataCallBack() {
            ...
            @Override
            public void requestSuccess(String result) throws Exception {
                Log.e("Setting Activity", "--->评估记录上传结果" + result);
                JSONObject toJsonObj= new JSONObject(result);
                int code = toJsonObj.getInt("code");
                boolean isDuplicate = false;
                if (code == 1){
...
//                    }
                    if (pageNum+1 >= listList.size()){
                        EventBus.getDefault().post(new MessageEvent<>(MessageEvent.ACTION_SYNC_EVALUATE_RECORD_TO_LOCAL_RESULT,isDuplicate? 0 : code));
                    }else {
            ...
            public void requestSuccess(String result) throws Exception {
                ...
                if (tokenBean.getCode() == 0){
                    ...
                    if (flag == PLAN_REQ){
                        ...
                        syncTrainRecord(currentUser != null ? currentUser.getUserId() : SPHelper.getUserId());
                        ...
                        syncEvaluateRecord(currentUser != null ? currentUser.getUserId() : SPHelper.getUserId());
                    }
                }
            }
        });
        ...
        switch (event.getAction()) {
            ...
            case MessageEvent.ACTION_SYNC_TRAIN_DATA_TO_LOCAL_RESULT:
                ...
                if (codeTrainData1 == 401){
                    ...
                    if (cacheQueue != null && !cacheQueue.isEmpty()){//队列中取用户数据，一个用户一个用户上传
                        currentUser = cacheQueue.poll();
                        syncTrainRecord(currentUser != null ? currentUser.getUserId() :  SPHelper.getUserId());
                    }else {
            ...
            case MessageEvent.ACTION_CHECK_VERSION:
                ...
                Toast.makeText(this,"已是最新版本了", LENGTH_SHORT).show();
//                ToastUtils.showShort("已是最新版本了");
            ...
            case MessageEvent.ACTION_SYNC_EVALUATE_RECORD_TO_LOCAL_RESULT:
                ...
                if (codeEvaluateData1 == 401){
                    ...
                    if (evaluateRecordUserQueue != null && !evaluateRecordUserQueue.isEmpty()){//队列中取用户数据，一个用户一个用户上传
                        currentUser = evaluateRecordUserQueue.poll();
                        syncEvaluateRecord(currentUser != null ? currentUser.getUserId() :  SPHelper.getUserId());
                    }else {
