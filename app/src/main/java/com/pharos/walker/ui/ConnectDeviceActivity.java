    ...
    private void initView() {
        ...
        bleDevAdapter.setOnItemClickListener(new BleDevAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BleBean bleBean) {
                ...
                Global.TempConnectedAddress = bleBean.getAddress();
...
//                    addConnectTimeout();
            ...
            case MessageEvent.ACTION_ClEAR_CONNECTED_DEVICE:
                refreshBle();
                break;
            case MessageEvent.ACTION_CONNECTED_TIMEOUT:
                refreshBle();
                BluetoothController.getInstance().disconnectGatt();
    ...
    private void startNextStep(){
        if (selectActivity == Global.ConnectMainMode){
            ...
            startTargetActivity(bundle,EvaluateStartActivity.class,true);
        }else {
            startTargetActivity(TrainParamActivity.class,true);
        }
    }
    ...
    private void addConnectTimeout(){
        if (mTimer1 == null && mTimerTask1 == null){
            ...
            mTimerTask1 = new TimerTask() {
                ...
                public void run() {
                    timeOutCount++;
                    if ( timeOutCount >= maxTimeoutTime){
                        EventBus.getDefault().post(new MessageEvent<>(MessageEvent.ACTION_CONNECTED_TIMEOUT));
                        cancelTimeoutTimer();
                        timeOutCount = 0;
                    }
                }
