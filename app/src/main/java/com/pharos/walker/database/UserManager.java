...
public class UserManager {
    ...
    public List<UserBean> loadAllByDate() {
        mUserDao.detachAll();
//        List<UserBean> userBeanList = mUserDao.queryBuilder().where(UserBeanDao.Properties.CaseHistoryNo.notEq("123456")).orderRaw("'getTimestamp(CREATE_DATE)' DESC" ).list();
    ...
    private long getTimestamp(String date){
        return DateFormatUtil.getString2Date(date);
    }
    /**
     * 查询所有用户信息
     *
...
//        userBean.setPingYin(MyFunc.toPinyin(userBean.getName()));
        if (mode == 0){
            if (TextUtils.isEmpty(userBean.getCreateDate())){
...
//            TrainPlanManager.getInstance().insertList(SnowflakeIdUtil.getUniqueId());
            ...
            if (userBean.getCardType() == 0 && TextUtils.isEmpty(userBean.getIdCard())){
                userBean.setIdCard(userBean.getCaseHistoryNo());
            }
            if (!userBean.getDate().equals(SPHelper.getUser().getDate()) ||
                    !userBean.getWeight().equals(SPHelper.getUser().getWeight()) ||
                    !userBean.getDiagnosis().equals(SPHelper.getUser().getDiagnosis())){
                userBean.setStr("plan");//使用备用数据库字段，设置是否清除计划的标志
            ...
            userBean.setIsUpload(Global.UploadStatus);
            mUserDao.update(userBean);
            if (userBean.getUserId() == SPHelper.getUserId()){
                SPHelper.saveUser(userBean);
            }
        }else if (mode == 3){
    ...
    public void insert(List<UserBean> userBeanList){
        if (userBeanList == null)
            return;
        for (UserBean userBean : userBeanList){
            userBean.setId(null);
            mUserDao.insertOrReplace(userBean);
        }
//        mUserDao.insertOrReplaceInTx(userBeanList);
    }
    /**
     * 添加新用户
     *
...
//        userBean.setPingYin(MyFunc.toPinyin(userBean.getName()));
    ...
    public void updateUserUploadStatus(UserBean userBean,int uploadStatus) {
        userBean.setIsUpload(uploadStatus);
        if (TextUtils.isEmpty(userBean.getCreateDate())){
            userBean.setCreateDate(DateFormatUtil.getNowDate());
        }
