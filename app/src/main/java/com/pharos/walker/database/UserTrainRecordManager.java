...
//        UserManager.getInstance().update(userBean);
    ...
    public void insertMany(List<UserTrainRecordEntity> userTrainRecordEntityList){
        if (userTrainRecordEntityList == null)
            return;
        for (UserTrainRecordEntity entity:userTrainRecordEntityList){
            entity.setId(null);
            userTrainRecordDao.insertOrReplace(entity);
        }
...
//        List<UserTrainRecordEntity> list = userTrainRecordDao.queryRaw(strSql,null);
    ...
    public  List<UserTrainRecordEntity> loadAll(){
        return userTrainRecordDao.loadAll();
    }
    ...
    public List<UserTrainRecordEntity> loadMasterUpdate(long userId){
        ...
        for (UserTrainRecordEntity trainRecordEntity : trainRecordEntities){
            if (trainRecordEntity.getIsUpload() != Global.UploadNetStatus){
                ...
                trainRecordEntity.setProductSn(SPHelper.getSerialNumber());
                trainRecordEntity.setSource("2");
    ...
    public List<UserTrainRecordEntity> loadByDate(long userId,String dateStr){
        return userTrainRecordDao.queryBuilder().where(UserTrainRecordEntityDao.Properties.UserId.eq(userId),
                UserTrainRecordEntityDao.Properties.DateStr.eq(dateStr)).orderAsc(UserTrainRecordEntityDao.Properties.CreateDate).list();
    }
    public int getTrainDay(long userId){
        ...
        for (UserTrainRecordEntity entity : userTrainRecordEntities){
            ...
            finalDate = entity.getDateStr();
        }
        if (linkedHashSet.size() > 0){
            if ((System.currentTimeMillis() - DateFormatUtil.getString2Date(finalDate + " 00:00:00")) > 24*60*60*1000){
